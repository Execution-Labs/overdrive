...
describe('App default route', () => {
  ...
  it('sends task_type=auto directly without calling classify-pipeline', async () => {
    const mockedFetch = global.fetch as unknown as ReturnType<typeof vi.fn>
    ...
    expect(createCall).toBeTruthy()
    const body = JSON.parse(String((createCall?.[1] as RequestInit).body))
    expect(body.task_type).toBe('auto')
    // No classify-pipeline call should have been made
    const classifyCalls = mockedFetch.mock.calls.filter(([url]) => String(url).includes('/api/tasks/classify-pipeline'))
    expect(classifyCalls.length).toBe(0)
  })

  it('does not include classifier metadata when submitting with auto type', async () => {
    const mockedFetch = global.fetch as unknown as ReturnType<typeof vi.fn>
    mockedFetch.mockImplementation((url, init) => {
      const u = String(url)
      const method = String((init as RequestInit | undefined)?.method || 'GET').toUpperCase()
    ...
    const body = JSON.parse(String((createCall?.[1] as RequestInit).body))
    expect(body.task_type).toBe('auto')
    expect(body.classifier_pipeline_id).toBeUndefined()
    expect(body.classifier_confidence).toBeUndefined()
    expect(body.classifier_reason).toBeUndefined()
    expect(body.was_user_override).toBeUndefined()
  })
