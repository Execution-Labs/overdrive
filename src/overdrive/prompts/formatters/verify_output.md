...
{{"status": "pass|fail|skip|environment", "reason_code": "assertion_failure|type_error|lint_violation|baseline_failure|scope_violation|tool_missing|config_missing|no_tests|permission_denied|resource_limit|os_incompatibility|infrastructure|unknown", "summary": "one-line summary of results"}}
...
- Use "baseline_failure" when failures are clearly pre-existing baseline debt outside the task's allowed scope and unchanged by the current task diff.
- Use "scope_violation" when verification evidence indicates files outside allowed scope were modified.
